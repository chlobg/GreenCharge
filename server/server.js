...
const PRICES = {
  ...
  VN: {
    AC: { day: 3100, night: 2700 },
    DC: { day: 3500, night: 3100 },
    currency: "VND",
  },
};
...
const priceAt = (country, type, date) => {
  const C = PRICES[country] || PRICES.FR;
  return {
    unit: isOffpeak(date) ? C[type].night : C[type].day,
    currency: C.currency,
  };
};
...
app.get("/geocode", async (req, res) => {
  try {
    ...
    const hit = data[0];
    const countryCode = hit?.address?.country_code?.toUpperCase?.() || "FR";
...
function pickBest(
  ...
  for (const s of stations) {
    ...
    const { unit: unitNow, currency } = priceAt(country, type, departDate);
    const nowCost = energyKWh * unitNow;
    ...
    const { unit: unitHC } = priceAt(country, type, nextHC);
    const hcCost = energyKWh * unitHC;
  ...
  try {
    ...
    if (E0 >= Eneed && !forceCharge) {
      return res.json({
        ...
        note: "no stop needed",
        country,
        currency: PRICES[country]?.currency || "EUR",
      });
    }
    ...
    const best = pickBest(stations, route.depart, Emin, 11, country);
    ...
    res.json({
      ...
      recommendation: {
        ...
        estimatedCost: +best.cost.toFixed(
          PRICES[country]?.currency === "VND" ? 0 : 2
        ),
